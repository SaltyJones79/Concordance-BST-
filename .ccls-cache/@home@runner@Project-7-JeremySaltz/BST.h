...
public:
  ...
  // Traverses a binary search tree in preorder, calling function Visit once for
  // each item. Precondition: The function represented by Visit exists outside
  // of the class implementation. Postcondition: Visit's action occurred once
  // for each item in the tree. Note: Visit can alter the tree.
...
private:
  ...
  // Precondition: NodePtr points to a node in a binary search tree; NodePtr !=
  // NULL. Postcondition: TreeItem contains the item in the leftmost descendant
  // of the node to which NodePtr points. The leftmost descendant of NodePtr is
  // deleted.

  void RetrieveItem(ptrType TreePtr, keyType SearchKey, treeItemType &TreeItem,
                    bool &Success) const;
