...
treeNode::treeNode(const treeItemType &NodeItem, ptrType L, ptrType R)
    : wordEntry(NodeItem), LChildPtr(L), RChildPtr(R) {} // end constructor
...
void bstClass::InsertItem(ptrType &TreePtr, const treeItemType &NewItem,
  ...
  else if (NewItem.Key() < TreePtr->wordEntry.Key())
...
// Calls: DeleteNodeItem.
{
  if (TreePtr == NULL)
    Success = false; // empty tree
           ...
           TreePtr->wordEntry.Key()) { // item is in the root of some subtree
    DeleteNodeItem(TreePtr);           // delete the item
    Success = true;
  } // end if in root

  // else search for the item
  else if (SearchKey < TreePtr->wordEntry.Key())
    // search the left subtree
...
void bstClass::CopyTree(ptrType TreePtr, ptrType &NewTreePtr) const {
  ...
  if (TreePtr != NULL) { // copy node
    NewTreePtr = new treeNode(TreePtr->wordEntry, NULL, NULL);
...
ptrType bstClass::RootPtr() const { return Root; } // end RootPtr

void bstClass::SetRootPtr(ptrType NewRoot) { Root = NewRoot; } // end SetRoot
